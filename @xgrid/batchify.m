...
% 
% ```matlab
% xg.batchify(params,param_names)
% ```
% 
% **Description**
%
% Specify parameters to use in the simulations, and specify parameter names
% 
% - **`xg.batchify(params,param_names)`** sets up parameters and parameter names specified in the `params` and `param_names` variables. `params` must be a 2D matrix and `parameter_names` must be a cell array specifying legal names in the xolotl object in `xg.x`
%
% See Also:
% xgrid.batchify
% xgrid.simulate
